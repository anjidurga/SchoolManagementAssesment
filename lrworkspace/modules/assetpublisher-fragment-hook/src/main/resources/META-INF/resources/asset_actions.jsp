...
<%@page import="com.liferay.schoolmng.school.service.permission.SchoolDataPermissionChecker"%>
...
PortletURL deletePortletURL = null;
...
if (showEditURL && assetRenderer.hasEditPermission(permissionChecker)) {
	...
	deletePortletURL = assetRenderer.getURLEdit(liferayPortletRequest, liferayPortletResponse, LiferayWindowState.POP_UP, redirectURL);
	
	deletePortletURL.setParameter("cmd", "delete");
}
	...
	<div class="pull-right">
		...
		>
			<c:if test="<%= editPortletURL != null %>">
				...
				<liferay-ui:icon
					data="<%= data %>"
					message='<%= showIconLabel ? LanguageUtil.format(request, "delete-x-x", new Object[] {"hide-accessible", HtmlUtil.escape(assetRenderer.getTitle(locale))}, false) : LanguageUtil.format(request, "delete-x", HtmlUtil.escape(assetRenderer.getTitle(locale)), false) %>'
					method="get"
					url="<%= deletePortletURL.toString() %>"
					useDialog="<%= true %>"
				/>
